...
function PageShell({ title, lede, children, onNavigate }) {
  ...
  return (
    <main dir="rtl" style={{ background: 'var(--surface-page)' }}>
      ...
      <div style={{ maxWidth: 1080, margin: '0 auto', padding: '40px 24px 8px' }}>
        <h1 style={{ margin: 0, fontFamily: 'var(--font-sans-ar)', fontSize: 32, fontWeight: 800, textTransform: 'none', letterSpacing: 'normal' }}>{title}</h1>
        {lede && <p style={{ margin: '10px 0 0', fontFamily: 'var(--font-sans-ar)', fontSize: 17, lineHeight: 1.9, color: 'var(--text-muted)', maxWidth: 640 }}>{lede}</p>}
  ...
  return (
    <PageShell onNavigate={onNavigate} title="من نحن" lede="مصنع ومعرض المروة لزخرفة الزجاج والكريستال — جراح، أجا، الدقهلية.">
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 32, alignItems: 'center' }}>
        <img src="/assets/storefront-night.jpg" alt="معرض المروة" style={{ width: '100%', height: 420, objectFit: 'cover', borderRadius: 'var(--radius-md)', border: '1px solid var(--border-hairline)' }} />
          ...
          <div style={{ display: 'flex', gap: 32, marginTop: 8 }}>
            {[['1000+', 'منتج في المعرض'], ['27', 'محافظة بنشحن ليها'], ['14 يوم', 'حق استبدال']].map(([v, l]) => (
              <div key={l} style={{ display: 'flex', flexDirection: 'column', gap: 4 }}>
                <span style={{ fontFamily: 'var(--font-sans-ar)', fontSize: 28, fontWeight: 900, color: 'var(--gold-600)' }}>{v}</span>
  ...
  return (
    <PageShell onNavigate={onNavigate} title="تواصل معنا" lede="اسأل عن أي منتج، أو اطلب سعر جملة. بنرد في نفس اليوم.">
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 380px', gap: 28, alignItems: 'start' }}>
        <div style={{ background: 'var(--marble-000)', border: '1px solid var(--border-hairline)', borderRadius: 'var(--radius-md)', padding: 24 }}>
          {sent ? (
